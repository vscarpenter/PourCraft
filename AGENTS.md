...
- **Platform:** iOS 17+ (SwiftUI), iPhone + first-class iPad
...
- **Testing:** Swift Testing framework (`import Testing`, `@Test`, `#expect`)
  plus XCTest UI screenshot coverage
- **Xcode:** 16.0+
...
# Test (unit + UI screenshot targets)
xcodebuild -project PourCraft.xcodeproj -scheme PourCraft -sdk iphonesimulator \
...
├── PourCraft/
│   ├── PourCraftApp.swift           # @main, iPhone tab shell + iPad sidebar, @AppStorage persistence
...
└── PourCraftUITests/
    └── PourCraftUITests.swift       # App Store screenshot generation and iPhone/iPad UI coverage
```

## Architecture Decisions

### Tab structure
- Compact width uses custom `ZineTabBar` (not SwiftUI's `TabView`); iPad wide
  layouts use `ZineSidebar`. Selected tab is `@State` in `PourCraftApp`; "Begin
  the brew" CTA on Brew swaps to Guide via that binding.
- Tips uses a compact `NavigationStack` on iPhone and a list/detail split
  composition on iPad.
